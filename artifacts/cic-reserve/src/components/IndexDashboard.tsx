...
export default function IndexDashboard() {
  ...
  return (
    <section id="index" ref={ref} className="py-32 bg-[#050505] relative">
            ...
            {assets.map((asset, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="glass-card rounded-sm p-4 group cursor-default transition-all duration-300"
                >
                  <div className="flex items-center gap-4">
                    ...
                    {/* Symbol */}
                    <span
                      className="text-xs font-mono font-bold tracking-widest w-16 flex-shrink-0"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="text-[#505050] text-xs ml-2">
                        {assetInfo.desc}
                      </span>
                    </div>

                    {/* Percentage */}
                    <span className="text-[#C4A77D] font-bold text-lg counter flex-shrink-0">
                      {asset.pct}%
                    </span>
                  </div>

                  {/* Progress bar */}
                  <div className="mt-3 h-px bg-[#1A1A1A] rounded-full overflow-hidden">
                    <motion.div
          ...
          >
            <div className="relative w-72 h-72 float">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-[#C4A77D] text-3xl font-bold">CIC</span>
                <span className="text-[#505050] text-xs tracking-widest uppercase mt-1">
                  Index
                </span>
              </div>
            </div>
            ...
            <div className="mt-8 grid grid-cols-3 gap-3 w-full max-w-xs">
              {assets.map((a) => (
                <div key={a.key} className="flex items-center gap-1.5">
                  ...
                  />
                  <span className="text-[#606060] text-xs">{a.symbol}</span>
                  <span className="text-[#C4A77D] text-xs font-bold ml-auto">{a.pct}%</span>
                </div>
              ))}
            </div>
          </motion.div>
